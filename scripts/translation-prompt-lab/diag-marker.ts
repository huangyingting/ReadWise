...
const db = openReadOnly("prisma/provider-dbs/workinprogress.db");
const rows = db.prepare("SELECT id, title, content, category FROM Article ORDER BY id LIMIT 8").all() as {
  id: string;
  title: string;
  content: string;
