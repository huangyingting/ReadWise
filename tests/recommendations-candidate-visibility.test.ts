...
function publicLibraryArticle(id: string, publishedAt = NOW): Article {
  return buildArticle({
    id,
...
async function recommendationIds(userId = "user-1", limit = 10): Promise<string[]> {
  const { listScoredPicksPage } = await import("@/lib/recommendations/picks");
  const page = await listScoredPicksPage(userId, { limit });
  return page.articles.map((a) => a.id);
}

for (const { name, articles, expected, message } of [
  {
    name: "private imported article (PRIVATE, owned)",
    articles: [
      publicLibraryArticle("public"),
      buildArticle({
        id: "private-import",
    ...
    expected: ["public"],
    message: "private imported article must never be a recommendation candidate",
  },
  {
    name: "another user's private article",
    articles: [
      publicLibraryArticle("public"),
      buildArticle({
        id: "user2-private",
    ...
    expected: ["public"],
    message: "another user's private article must never be a recommendation candidate",
  },
  {
    name: "draft article (PUBLIC but DRAFT status)",
    articles: [
      publicLibraryArticle("published"),
      buildArticle({
        id: "draft",
        visibility: ArticleVisibility.PUBLIC,
    ...
    expected: ["published"],
    message: "draft article must never be a recommendation candidate",
  },
  {
    name: "owned-public article (PUBLIC but ownerId set)",
    articles: [
      publicLibraryArticle("library"),
    ...
    expected: ["library"],
    message: "owned-public article must never be a recommendation candidate",
  },
]) {
  test(`IDOR: ${name} is never a recommendation candidate`, async () => {
    articleRows = articles;
    assert.deepEqual(await recommendationIds(), expected, message);
  });
}
...
test("IDOR: candidate set is user-agnostic — same public-only articles for every user", async () => {
  ...
  articleRows = [
    publicLibraryArticle("lib-a", new Date("2026-06-25T00:00:00Z")),
    publicLibraryArticle("lib-b", new Date("2026-06-24T00:00:00Z")),
...
test("series extra candidate: an accessible public id is injected and scored", async () => {
  ...
  articleRows = [
    publicLibraryArticle("public"),
    publicLibraryArticle("series-public"),
  ];
