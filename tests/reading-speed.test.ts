...
describe("clampActiveTime", () => {
  test("clamps negative values to 0", async () => {
    const { clampActiveTime } = await import("@/lib/reading-speed");
  ...
  test("clamps values above MAX_ACTIVE_TIME_MS", async () => {
    const { clampActiveTime, MAX_ACTIVE_TIME_MS } = await import("@/lib/reading-speed");
...
describe("computeWpm", () => {
  test("returns null for null wordCount", async () => {
    const { computeWpm } = await import("@/lib/reading-speed");
  ...
  test("returns null when active time is below MIN_ACTIVE_TIME_MS", async () => {
    const { computeWpm, MIN_ACTIVE_TIME_MS } = await import("@/lib/reading-speed");
  ...
  test("clamps absurdly high WPM to MAX_WPM", async () => {
    const { computeWpm, MAX_WPM } = await import("@/lib/reading-speed");
  ...
  test("clamps absurdly slow WPM to MIN_WPM", async () => {
    const { computeWpm, MIN_WPM } = await import("@/lib/reading-speed");
...
describe("computeWpmTrend", () => {
  test("returns all-null when records list is empty", async () => {
    const { computeWpmTrend } = await import("@/lib/reading-speed");
