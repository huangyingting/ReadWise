...
function quizDataset(cases: EvalDataset["cases"]): EvalDataset {
  return { feature: "quiz", cases };
}

function formatEvalFailures(failures: ReturnType<typeof collectFailures>): string {
  return `eval property failures:\n${failures
...
test("a missing modelOutput is reported as a failed property in offline mode", async () => {
  const noOutput = quizDataset([{ name: "no-output", input: { title: "T", source: "S" } }]);
