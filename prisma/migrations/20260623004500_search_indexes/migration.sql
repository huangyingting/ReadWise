-- Replace the SQLite FTS5 article search path with portable, application-ranked
-- search. These drops are safe for local/dev databases that never applied the
-- prior FTS migration.
...
-- Core feed/search/worker/admin query indexes. Keep names explicit so schema,
-- docs, and query-plan tests can refer to the same contract.
