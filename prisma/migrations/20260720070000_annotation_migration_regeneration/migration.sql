-- SQLite migration for Phase 3.4 annotation re-anchoring + derived-output
...
-- Both are nullable and additive; Json is stored as JSONB (TEXT under SQLite).
-- The actual anchor OFFSET migration + derived-output regeneration are DATA
-- operations (highlight updates, cache clears, deduplicated jobs) that need no
-- schema change.
