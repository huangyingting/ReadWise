...
| Search | `searchPublishedArticles`, `postgresTextMatches`, annotation/vocab merge | `Article_search_vector_idx`, feed indexes, `Highlight_user_created_idx`, `SavedWord_user_article_idx` | Candidate sets are capped; PostgreSQL FTS ranks article body/title/excerpt while app ranking adds byline/annotation boosts. |
