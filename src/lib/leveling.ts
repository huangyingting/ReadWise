/**
 * Adaptive CEFR level progression — #37.
 *
 * Computes a recommendation to level up or down based on:
 *   - Recent quiz scores for articles at/above the current level
 *   - Reading completion rate for articles at the current level
 *   - Sparse-data guard (returns "hold" when signals are insufficient)
 *
 * All functions are PURE (no DB access) so they are unit-testable in isolation.
 * DB access lives in `getLevelingSignals` which is called by the API route.
 */
