...
import { extractReadable, type ReadableArticle } from "@/lib/scraper/readability-extract";
...
function paragraphsToHtml(
  text: string,
  policy?: ProviderExtractionPolicy,
): string {
  const paragraphs = text
    .split(/\n{2,}|\r\n\r\n/)
    .map((p) => p.trim())
    .filter(Boolean);
  const normalized = policy?.normalizeJsonLdParagraphs?.(paragraphs) ?? paragraphs;
  return normalized
    .map((p) => `<p>${p.replace(/\n/g, " ")}</p>`)
    .join("\n");
}
...
function chooseArticleBody(
  ...
  if (!ldBody) {
    ...
    const overTrimmed = legacyWords > readable.wordCount * READABILITY_LEGACY_MAX_WORD_RATIO;
    return overTrimmed ? legacyBody : readable.contentHtml;
  }
  ...
  if (
    policy?.preferReadabilityForCollapsedJsonLd &&
    readableLongEnough &&
...
export function extractArticle(html: string, sourceUrl: string): ScrapedArticle | null {
  ...
  const legacyBody = ldBody
    ? paragraphsToHtml(ldBody, provider?.extraction)
    : extractBodyHtml(bodyHtml, sourceUrl);
  ...
  const chosenBody = chooseArticleBody(legacyBody, ldBody, readable, provider?.extraction);
