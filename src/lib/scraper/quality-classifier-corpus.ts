/**
 * Seed training corpus for the local Naive-Bayes ad/article quality classifier
 * (Issue #739 follow-up).
 *
 * This is a small, hand-curated bootstrap corpus used by
 * `scripts/train-quality-classifier.ts` to build the committed model JSON
 * (`quality-classifier-model.json`). Labels are heuristic-aligned: `article`
 * samples are genuine prose across varied topics; `ad` samples are promo copy,
 * navigation/link-list fragments, cookie/consent banners, and classified-ad
 * style text — the kinds of junk the scraper should down-rank.
 *
 * @server-only — only consumed by the training script and the (server-only)
 * classifier module. Contains NO user-private content; all samples are
 * synthetic, representative public-style text.
 *
 * PRIVACY: never add real scraped article bodies or any user content here.
 */
...
export const AD_SAMPLES: readonly string[] = [
