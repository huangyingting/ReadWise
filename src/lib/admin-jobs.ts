/**
 * Admin job dashboard helpers (RW-017).
 *
 * Read-side queries (paginated listing + aggregate counts + stuck/failed
 * surfacing) and safe admin actions (retry / cancel / archive) over the
 * persistent `Job` table. The heavy lifting (locking, retry policy, lifecycle
 * transitions) lives in `@/lib/jobs`; this module shapes that into the operator
 * dashboard and enforces which actions are safe for a given job status.
 */
...
// ---------------------------------------------------------------------------
// Admin actions (retry / cancel / archive) with per-status safety guards.
// ---------------------------------------------------------------------------
...
export type JobActionResult =
  | { ok: true; job: Job; previousStatus: string; action: JobActionName }
 ...
 *
 * Returns a structured result the route layer turns into a status code + audit.
 */
  ...
  switch (action) {
    case "retry": {
      if (job.status !== JobStatus.FAILED && job.status !== JobStatus.DEAD_LETTER) {
        return { ok: false, status: 409, error: `Cannot retry a ${job.status} job` };
      }
      const updated = await retryJob(jobId);
      if (!updated) return { ok: false, status: 404, error: "Job not found" };
      return { ok: true, job: updated, previousStatus, action };
    }
    case "cancel": {
      if (isTerminal) {
        return { ok: false, status: 409, error: `Cannot cancel a ${job.status} job` };
      }
    ...
    case "archive": {
      if (!isTerminal) {
        return {
          ok: false,
          status: 409,
          error: `Cannot archive a ${job.status} job (only completed or dead-letter jobs)`,
        };
      }
      const removed = await archiveJob(jobId);
      if (!removed) return { ok: false, status: 404, error: "Job not found" };
      return { ok: true, job: removed, previousStatus, action };
    }
    default: {
