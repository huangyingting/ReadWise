 ...
 *
 * Thin helpers that all layers can use without pulling in the Prisma client
 * singleton. Keeping these separate from `@/lib/prisma` ensures
...
import { databaseProviderFromUrl } from "@/lib/database-provider-policy.mjs";

export function hasPostgresUrlPrefix(url: string): boolean {
  return databaseProviderFromUrl(url) === "postgresql";
}
