...
import { aiCostConfig, aiLedgerEnabled, type AiCostRate } from "@/lib/config";
...
// ---------------------------------------------------------------------------
// Aggregation (for future admin analytics — RW-019 acceptance criteria)
// ---------------------------------------------------------------------------
 ...
 * and status within an optional time range. Intended for admin analytics.
 */
export async function summarizeAiUsage(
  ...
  const [aggregate, fallbackCount, cacheHitCount, byFeature, byModel, byStatus] = await Promise.all([
