/**
 * Admin tag mutation commands — article-library subsystem (ADR-0010 §6).
 *
 * Tag and ArticleTag records are owned by the Article Library. Rename, merge,
 * and delete operations that span Tag + ArticleTag + AuditLog are kept here so
 * transaction boundaries and ownership predicates live in one place.
 *
 * The admin read-model (`listAdminTags`) also lives here because it shapes Tag
 * data for the admin UI and uses Article Library access predicates
 * (`publicListableArticleWhere`).
 *
 * Routes in `src/app/api/admin/tags/` delegate to these commands; they do NOT
 * own business logic or transaction boundaries.
 */
...
import { publicListableArticleWhere } from "./policy";

// ---------------------------------------------------------------------------
// Admin tag read model
// ---------------------------------------------------------------------------
 ...
 * Matches the query (case-insensitively via SQLite LIKE) against name and slug,
 * and includes usage counts: total articles carrying the tag plus how many of
 * those are published. Paginated, ordered by total usage (most-used first),
 * then alphabetically.
 */
 ...
 *
 * Transaction boundary: Tag update + optional AuditLog write are atomic.
 */
 ...
 * don't already exist on the target are re-pointed to the target; duplicates
 * are silently skipped. The source tag is then deleted (its remaining
 * ArticleTag rows are cascade-deleted). Runs in a single DB transaction.
 *
 * Transaction boundary: ArticleTag re-link + Tag delete + optional AuditLog
 * write are atomic; a partial failure leaves no orphaned links.
 */
 ...
 *
 * Transaction boundary: Tag delete + optional AuditLog write are atomic.
 */
