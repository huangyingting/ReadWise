...
export const BACKFILL_FEATURES = [
  "difficulty",
  "tags",
  "vocabulary",
  "quiz",
...
export type BackfillFeature = (typeof BACKFILL_FEATURES)[number];
...
export function isBackfillFeature(value: string): value is BackfillFeature {
  return (BACKFILL_FEATURES as readonly string[]).includes(value);
}
...
function missingStepKeys(
  ...
  for (const feature of features) {
    if (feature === "difficulty" && article.difficulty == null) out.push("difficulty");
    else if (feature === "tags" && article._count.tags === 0) out.push("tags");
    else if (feature === "vocabulary" && article._count.vocabulary === 0) out.push("vocabulary");
    else if (feature === "quiz" && article._count.quizQuestions === 0) out.push("quiz");
    else if (feature === "grammar" && article._count.grammarExplanations === 0) out.push("grammar");
    else if (feature === "speech" && !article.speech) out.push("speech");
