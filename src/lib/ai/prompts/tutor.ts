...
function renderTutorSystemPrompt({ level, title, articleText }: TutorPromptVars): string {
  return (
...
function renderTutorQuestion(question: string): string {
  return sanitizeUntrustedText(question);
}
...
const tutorTemplate: PromptTemplate<TutorPromptVars> = {
  ...
  render: ({ level, title, articleText, question }) => [
    {
      role: "system",
      content: renderTutorSystemPrompt({ level, title, articleText, question }),
    },
    {
      role: "user",
      content: renderTutorQuestion(question),
    },
  ],
};
