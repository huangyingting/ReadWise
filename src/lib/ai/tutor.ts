/**
 * AI Tutor commands — AI subsystem (ADR-0010 §6).
 *
 * Provides a per-user, per-article conversational tutor grounded in the
 * article text. Conversation turns are persisted in `TutorMessage` via an
 * explicit transaction boundary; fallback responses (AI unconfigured /
 * request failure) are NEVER persisted.
 *
 * Previously at `src/lib/tutor.ts` (flat helper). Moved here so the
 * multi-model write and AI-provider orchestration live in the AI subsystem
 * that owns them (Phase 3, #686).
 */
