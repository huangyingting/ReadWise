/**
 * Prisma full-text search provider (ARCH-15).
 *
 * `PrismaArticleSearchProvider` — portable Prisma `contains` provider
 * (SQLite-safe; PostgreSQL falls back to this when semantic/FTS fails).
 ...
 *
 * The PostgreSQL path consumes `readableArticleSqlPredicate` from the Article
 * Library policy module. Its SQL and Prisma adapters render the same canonical
 * access expression, so FTS cannot drift from ordinary article reads.
 */
...
async function postgresTextMatches(
  ...
  try {
    const visibility = readableArticleSqlPredicate(access);
