...
function formatDueFlashcardLabel(dueCount: number): string {
  return `${dueCount} flashcard${dueCount === 1 ? "" : "s"} due for review`;
}
...
export function DashboardProgressBand({
  ...
  return (
    <section aria-labelledby="progress-h" className="mt-[var(--space-7)]">
      ...
      {dueCount > 0 ? <DueReviewCallout dueCount={dueCount} /> : null}
...
function DueReviewCallout({ dueCount }: { dueCount: number }) {
  return (
        ...
        <div className="flex items-center gap-[var(--space-3)]">
          ...
          <div>
            <p className="font-semibold text-text m-0">
              {formatDueFlashcardLabel(dueCount)}
            </p>
